...
class PolicyDecoder(nn.Module):
  def __init__(self, out_channels=16, final_channel_factor=1):
    ...
    self.net = nn.Sequential(
        # 8
        nn.ConvTranspose2d(final_channel_factor*out_channels,
                           4*out_channels, 3, padding=1),
        act_fn,
        ...
        act_fn,
        nn.ConvTranspose2d(8*out_channels, 8*out_channels,
                           3, padding=1),  # 128
        act_fn,
        nn.ConvTranspose2d(8*out_channels, 4*out_channels, 3, padding=1,
                           stride=2, output_padding=1),  # 256
        act_fn,
        nn.ConvTranspose2d(4*out_channels, 4*out_channels,
                           3, padding=1),  # 256
        act_fn,
  ...
  def __init__(self, out_channels=16, final_channel_factor=1):
    ...
    self.net = nn.Sequential(
        ...
        nn.Conv2d(4*out_channels, 4*out_channels,
                  3, padding=1, stride=2),  # 16
        act_fn,
        nn.Conv2d(4*out_channels, final_channel_factor * \
                  out_channels, 3, padding=1, stride=2)  # 8
    )
...
class SmallerPolicyDecoder(nn.Module):
  def __init__(self, out_channels=16, final_channel_factor=1):
    ...
    act_fn = nn.ReLU()
    self.net = nn.Sequential(
        nn.ConvTranspose2d(final_channel_factor*out_channels, 4 *
                           out_channels, 3, padding=1, stride=2, output_padding=1),  # 16
        act_fn,
        nn.ConvTranspose2d(4*out_channels, 4*out_channels, 3,
                           padding=1, stride=2, output_padding=1),  # 32
        act_fn,
        nn.ConvTranspose2d(4*out_channels, 4*out_channels, 3,
                           padding=1, stride=2, output_padding=1),  # 64
        act_fn,
        nn.ConvTranspose2d(4*out_channels, 4*out_channels, 3,
                           padding=1, stride=2, output_padding=1),  # 128
        act_fn,
        nn.ConvTranspose2d(4*out_channels, 2, 3, padding=1,
                           stride=2, output_padding=1),  # 256
    )
  ...
  def __init__(self, in_channels=3, out_channels=16, latent_dim=200):
    ...
    self.net = nn.Sequential(
        ...
        nn.Conv2d(4*out_channels, 4*out_channels,
                  3, padding=1, stride=2),  # 8
        nn.Flatten(),
        nn.Linear(8*8*4*out_channels,
                  latent_dim) if latent_dim is not None else nn.Identity(),
    )
...
class BasicCNNDecoder(nn.Module):
  def __init__(self, in_channels=3, out_channels=16, latent_dim=200):
    ...
    self.net = nn.Sequential(
        nn.Linear(latent_dim, 4*out_channels*8 *
                  8) if latent_dim is not None else nn.Identity(),
...
class PolicyAutoencoder(nn.Module):
  def __init__(self, out_channels=16, final_channel_factor=1):
    ...
    self.encoder = PolicyEncoder(
        out_channels=out_channels, final_channel_factor=final_channel_factor)
...
class SmallerPolicyAutoencoder(nn.Module):
  def __init__(self, out_channels=16, final_channel_factor=1):
    super().__init__()
    self.encoder = SmallerPolicyEncoder(
        out_channels=out_channels, final_channel_factor=final_channel_factor)
    self.decoder = SmallerPolicyDecoder(
        out_channels=out_channels, final_channel_factor=final_channel_factor)
