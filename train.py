...
def train_step(states, model, optimizer, batch_size, pool: mp.Pool, device='cpu', standard_dev=0.01, surface_batches=None):
  with torch.no_grad():
    ...
    noises = [[torch.randn_like(p, device=device) for p in model.parameters()] for _ in range(batch_size // 2)]
    noises = noises + [[-noise for noise in noise_list] for noise_list in noises]
    ...
    while running:
      ...
      for m, surface_batch in zip(models, surface_batches):
        ...
        hit_direction_batch = [Vec2(*d.cpu().numpy().tolist()) for d in hit_direction_batch]
      ...
      if print_timings:
        print("Check")
      check_start = time.time()
      running = any(any(not is_done(state) for state in state_batch) for state_batch in states_batches)
    ...
    losses = [sum([state_loss(state) for state in state_batch]) / len(state_batch) for state_batch in states_batches]
    ...
    for param_index in range(len(grad)):
      for loss_index, loss in enumerate(losses_reranked):
        grad[param_index] += (loss * noises[loss_index][param_index]) / (batch_size * standard_dev)
...
if __name__ == '__main__':
  ...
  states = [make_state() for _ in range(states_per_batch)]
  ...
  for i in range(400):
    ...
    losses, train_surfaces = train_step(states, model, optimizer, batch_size, pool, device=device, standard_dev=0.01, surface_batches=train_surfaces)
    # sort the losses
    ...
    loss, eval_surface = eval_batched([deepcopy(states[0])], model, device, pool, eval_surface)
